...
### This script appends symbols to the bfr_capacitors library and spice models files to the
### `spice_models/bfr_capacitors` folder.
...
### ("capacitor_generator.py") and the name of the part family definition file from which the
### symbol comes.
###
...
for family_spec in sys.argv[1:]:
    ...
    sym_lib_code = get_file_as_string(
        os.path.join(os.path.split(dir)[0], "symbols", "bfr_capacitors.kicad_sym")
    )
    ...
    # Write back library
    lib_file = open(
        os.path.join(os.path.split(dir)[0], "symbols", "bfr_capacitors.kicad_sym"), "w"
    )
